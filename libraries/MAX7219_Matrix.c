...
// NOTE TO SELF:
// 		If you want more, then add more variables first	
//	  	and then follow framebuffer instructions
//		with each variable chaining on each other
void MAX7219_MatrixRShift(uint8_t step)
{
										// added
	uint8_t msbD0, msbD1, msbD2, msbD3, msbD4, msbD5;	
	...
	{				
		...
		msbD4 = ((FrameBuffer[3][i] & 0x80) >> 7);
		FrameBuffer[4][i] = FrameBuffer[4][i] << 1;
		FrameBuffer[4][i] |= msbD3;					
		FrameBuffer[0][i] |= msbD4;

		msbD5 = ((FrameBuffer[3][i] & 0x80) >> 7);
		FrameBuffer[5][i] = FrameBuffer[5][i] << 1;
		FrameBuffer[5][i] |= msbD4;					
		FrameBuffer[0][i] |= msbD5;
	}	
}

// NOTE TO SELF:
// 		attempt to solve shifting of the lsD4 and lsD5
// 
// 
// 
...
{
										// added 
	uint8_t lsbD0, lsbD1, lsbD2, lsbD3, lsbD4, lsbD5;
	...
	for(int i=0; i < 8; i++)
	{
		...
		// lsbD5 = ((FrameBuffer[5][i] & 0x01) << 7);
		// FrameBuffer[5][i] = FrameBuffer[5][i] >> 1;

		// lsbD4 = ((FrameBuffer[4][i] & 0x01) << 7);
		// FrameBuffer[4][i] = FrameBuffer[4][i] >> 1;
		// FrameBuffer[4][i] |= lsbD5;

		// lsbD3 = ((FrameBuffer[3][i] & 0x01) << 7);
		// FrameBuffer[3][i] = FrameBuffer[3][i] >> 1;
		// FrameBuffer[3][i] |= lsbD4;
